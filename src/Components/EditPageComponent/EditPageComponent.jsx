...
setShowImage(false)
  ...
  const newsIdValidator = async () => {
    ...
    await axios.post(`${Basic_Url}/validate`, { ID: newsInfo.newsID })
      .then(response => {
  ...
  const formSubmitHandler = async (event) => {
    ...
    await axios.patch(`${Basic_Url}`, formData)
      .then(response => {
  ...
  return (
    <form className='form-container' onSubmit={formSubmitHandler}>
      ...
      { showImage && newsImage && <img src={newsImage} alt="image" width="300px" />}

      <div className='form-group'>
        <label>To edit image,upload here!</label>
        <input
          type='file'
          onClick={disableImage}
